...
<sec:authorize access="hasRole('ROLE_NOC')">
	<form:form modelAttribute="physicalInfrastructure" action="" method="post">
		<!-- Graphical view -->
		<div id="home_body">
			<!-- WAN -->
			<div id="wan">
								...
								<c:forEach items="${physicalInfrastructure.physicalRouterCore.interfaces}" varStatus="vs" var="item">											
									<c:choose>
										<c:when test="${item.type == 'Loopback'}">
											<div><b><spring:message code="interface"/>:&nbsp;</b><form:label path="physicalRouterCore.interfaces[${vs.index}].name" >${item.name}</form:label></div>		
										</c:when>
									</c:choose>
								</c:forEach>
							</div>
					...
					<c:forEach items="${physicalInfrastructure.physicalRouterMaster.interfaces}" varStatus="vs" var="item">											
						<c:choose>
							<c:when test="${item.type == 'Up'}">
								<div><b><spring:message code="interface"/>:&nbsp;</b><form:label path="physicalRouterMaster.interfaces[${vs.index}].name" >${item.name}</form:label></div>		
							</c:when>
						</c:choose>
					</c:forEach>
				</div>										
			...
			<div id="wan_backup" class="ui-widget-content">				
				...
				<div class="ui-widget-content config_content">
					<c:forEach items="${physicalInfrastructure.physicalRouterBackup.interfaces}" varStatus="vs" var="item">											
						<c:choose>
							<c:when test="${item.type == 'Up'}">
								<div><b><spring:message code="interface"/>:&nbsp;</b><form:label path="physicalRouterBackup.interfaces[${vs.index}].name" >${item.name}</form:label></div>		
							</c:when>
						</c:choose>
					</c:forEach>
				</div>		
