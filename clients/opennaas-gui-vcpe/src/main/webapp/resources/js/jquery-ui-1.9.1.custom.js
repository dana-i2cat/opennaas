			...
			.extend(
					$.ui,
					{
						...
						isOverAxis : function(x, reference, size) {
							// Determines when x coordinate is over "b" element
							// axis
							return (x > reference) && (x < (reference + size));
						},
						isOver : function(y, x, top, left, height, width) {
							// Determines when x, y coordinates is over "b"
							// element
...
(function($, undefined) {
	...
	$.fn.position = function(options) {
		...
		var atOffset, targetWidth, targetHeight, targetOffset, basePosition, target = $(options.of), within = $.position
				...
				.getScrollInfo(within), targetElem = target[0], collision = (options.collision || "flip")
				.split(" "), offsets = {};
	...
	if ($.uiBackCompat !== false) {
		...
		(function($) {
			...
			$.fn.position = function(options) {
				...
				var offset = options.offset.split(" "), at = options.at
						.split(" ");
				if (offset.length === 1) {
					offset[1] = offset[0];
				}
					...
					$.ui.mouse,
					{
							...
							if (this._helper) {
								...
								var s = {
									...
									height : (that.helper.height() - soffseth)
										...
										10) + (that.position.left - that.originalPosition.left))
						...
						_renderProxy : function() {
							...
							if (this._helper) {
								...
								var ie6offset = ($.ui.ie6 ? 1 : 0), pxyoffset = ($.ui.ie6 ? 2
										: -1);
			...
			.add(
					...
					"containment",
					{
						...
						resize : function(event, ui) {
							...
							var woset = Math
									.abs((that._helper ? that.offset.left
											- cop.left
											: (that.offset.left - cop.left))
											+ that.sizeDiff.width), hoset = Math
									.abs((that._helper ? that.offset.top
											- cop.top
											: (that.offset.top - co.top))
											+ that.sizeDiff.height);

							var isParent = that.containerElement.get(0) == that.element
						...
						stop : function(event, ui) {
							...
							var helper = $(that.helper), ho = helper.offset(), w = helper
									.outerWidth()
			...
			.widget(
					...
					$.ui.mouse,
					{
						...
						_mouseCapture : function(event, overrideHandle) {
							...
							var currentItem = null, nodes = $(event.target)
									.parents()
									.each(
											function() {
												if ($.data(this,
						...
						_intersectsWithPointer : function(item) {
							...
							var isOverElementHeight = (this.options.axis === 'x')
									...
									|| $.ui.isOverAxis(this.positionAbs.left
											+ this.offset.click.left,
							...
							if (!o.placeholder
								...
								o.placeholder = {
									element : function() {
										...
										var el = $(
												document
														.createElement(that.currentItem[0].nodeName))
												.addClass(
														className
						...
						_contactContainers : function(event) {
							...
							if (this.containers.length === 1) {
								...
								var posProperty = this.containers[innermostIndex].floating ? 'left'
										: 'top';
								var sizeProperty = this.containers[innermostIndex].floating ? 'width'
										: 'height';
...
(function($, undefined) {
	...
	$
			.widget(
					"ui.accordion",
					{
						...
						refresh : function() {
							var maxHeight, overflow, heightStyle = this.options.heightStyle, parent = this.element
									.parent();

							if (heightStyle === "fill") {
								// IE 6 treats height like minHeight, so we need
								// to turn off overflow
								// in order to get a reliable height
								// we use the minHeight support test because we
								// assume that only
								// browsers that don't support minHeight will
								// treat height as minHeight
	...
	if ($.uiBackCompat !== false) {
		...
		(function($, prototype) {
			...
			prototype._create = function() {
				if (this.options.navigation) {
					var that = this, headers = this.element
							.find(this.options.header), content = headers
							.next(), current = headers.add(content).find("a")
		...
		(function($, prototype) {
			$.extend(prototype.options, {
				heightStyle : null, // remove default so we fall back to old
									// values
			...
			$.extend(prototype, {
				...
				_setOption : function(key) {
					if (key === "autoHeight" || key === "clearStyle"
							|| key === "fillSpace") {
		...
		(function($, prototype) {
			$.extend(prototype.options.icons, {
				activeHeader : null, // remove default so we fall back to old
										// values
			...
			.widget(
					"ui.autocomplete",
					{
						...
						_create : function() {
							...
							this.valueMethod = this.element[this.element
							...
							this.menu = $("<ul>").addClass("ui-autocomplete")
									.appendTo(
											this.document
													.find(this.options.appendTo
															|| "body")[0])
									.menu({
										// custom key handling for now
										input : $(),
										// disable ARIA support, the live region
										// takes care of that
										role : null
									}).zIndex(this.element.zIndex() + 1).hide()
									.data("menu");
											...
											this.menu.element,
											{
												...
												menuselect : function(event, ui) {
													...
													var item = ui.item
															.data("ui-autocomplete-item")
															|| ui.item
																	.data("item.autocomplete"), previous = this.previous;
						...
						_setOption : function(key, value) {
							...
							if (key === "appendTo") {
								this.menu.element.appendTo(this.document
										.find(value || "body")[0]);
							}
			...
			.extend(
					Datepicker.prototype,
					{
						...
						_generateHTML : function(inst) {
							...
							html += buttonPanel
									+ ($.ui.ie6 && !inst.inline ? '<iframe src="javascript:false;" class="ui-datepicker-cover" frameborder="0"></iframe>'
											: '');
					...
					"ui.dialog",
					{
						...
						_create : function() {
							...
							uiDialogTitle = $("<span>").uniqueId().addClass(
									"ui-dialog-title").html(title).prependTo(
									uiDialogTitlebar);
						...
						_destroy : function() {
							...
							next = oldPosition.parent.children().eq(
									oldPosition.index);
						...
						open : function() {
							...
							this.overlay = options.modal ? new $.ui.dialog.overlay(
									this)
									: null;
							this.moveToTop(true);
						...
						_size : function() {
							...
							if (options.height === "auto") {
								...
								if ($.support.minHeight) {
									...
									this.uiDialog.show();
									autoHeight = this.element.css("height",
											"auto").height();
									if (!isVisible) {
										this.uiDialog.hide();
									}
			...
			.extend(
					$.ui.dialog.overlay,
					{
						...
						create : function(dialog) {
							...
							var $el = (this.oldInstances.pop() || $("<div>")
							...
							$(document)
									.bind(
											...
											function(event) {
												...
												// only react to the event if
												// we're the top overlay
												if (instances.length !== 0
														&& instances[instances.length - 1] === $el
						...
						refresh : function() {
							...
							submenus
									.each(function() {
										...
										menu.attr("aria-labelledby", item
												.attr("id"));
									});
			...
			.widget(
					"ui.progressbar",
					{
						...
						_refreshValue : function() {
							var value = this.value(), percentage = this
									._percentage();
...
(function($, undefined) {
	...
	function isLocal(anchor) {
		return anchor.hash.length > 1
				&& anchor.href.replace(rhash, "") === location.href.replace(
						rhash, "");
	}
	...
	$
			.widget(
					"ui.tabs",
					{
						...
						_create : function() {
							var that = this, options = this.options, active = options.active, locationHash = location.hash
									.substring(1);
							...
							if (this.options.active !== false
									&& this.anchors.length) {
								this.active = this
										._findActive(this.options.active);
							} else {
								this.active = $();
							}
						...
						_setupHeightStyle : function(heightStyle) {
							var maxHeight, overflow, parent = this.element
									.parent();
						...
						_destroy : function() {
							...
							this.anchors.removeClass("ui-tabs-anchor")
									.removeAttr("role").removeAttr("tabIndex")
									.removeData("href.tabs").removeData(
											"load.tabs").removeUniqueId();
						...
						// TODO: Remove this function in 1.10 when ajaxOptions
						// is removed
	...
	if ($.uiBackCompat !== false) {
			...
			_ajaxSettings : function(anchor, event, ui) {
				...
				return $.extend({}, ajaxOptions, {
					error : function(xhr, status) {
						try {
							// Passing index avoid a race condition when this
							// method is
							// called after the user has selected another tab.
							// Pass the anchor that initiated this request
							// allows
							// loadError to manipulate the tab content panel via
							// $(a.hash)
							ajaxOptions.error(xhr, status, ui.tab.closest("li")
									.index(), ui.tab[0]);
						} catch (error) {
						}
					}
		...
		$.widget("ui.tabs", $.ui.tabs, {
			...
			_create : function() {
				...
				this._on({
					tabsbeforeload : function(event, ui) {
						// Don't react to nested tabs or tabs that don't use a
						// spinner
		...
		$.widget("ui.tabs", $.ui.tabs, {
			...
			enable : function(index) {
				...
				if (index
						...
						|| ($.isArray(options.disabled) && $.inArray(index,
								options.disabled) !== -1)) {
					trigger = true;
				}
				...
				.widget(
						...
						$.ui.tabs,
						{
							...
							remove : function(index) {
								index = this._getIndex(index);
								var options = this.options, tab = this.tabs.eq(
										index).remove(), panel = this
										._getPanelForTab(tab).remove();

								// If selected tab was removed focus tab to the
								// right or
								// in case the last tab was removed the tab to
								// the left.
								// We check for more than 2 tabs, because if
								// there are only 2,
								// then when we remove this tab, there will only
								// be one tab left
								// so we don't need to detect which tab to
								// activate.
								...
								options.disabled = $.map($.grep(
										options.disabled, function(n) {
											return n !== index;
										}), function(n) {
									return n >= index ? --n : n;
								});
								...
								this._trigger("remove", null, this._ui(tab
										.find("a")[0], panel[0]));
								return this;
							}
						});
		...
		$.widget("ui.tabs", $.ui.tabs, {
			...
			_createPanel : function(id) {
				return $(this.options.panelTemplate).attr("id", id).addClass(
						"ui-tabs-panel ui-widget-content ui-corner-bottom")
						.data("ui-tabs-destroy", true);
			}
		});
		...
		$.widget("ui.tabs", $.ui.tabs, {
			_create : function() {
				...
				if (options.active === null && options.selected !== undefined) {
					options.active = options.selected === -1 ? false
							: options.selected;
				}
				this._super();
		...
		$.widget("ui.tabs", $.ui.tabs, {
			...
			_create : function() {
				this._super();
				if (this.options.active !== false) {
					this._trigger("show", null, this._ui(this.active
							.find(".ui-tabs-anchor")[0], this
		...
		(function() {
			...
			$.widget("ui.tabs", $.ui.tabs, {
				options : {
					cookie : null
				// e.g. { expires: 7, path: '/', domain: 'jquery.com', secure:
				// true }
				},
				_create : function() {
		...
		// fx option
		// The new animation options (show, hide) conflict with the old show
		// callback.
		// The old fx option wins over show/hide anyway (always favor
		// back-compat).
		// If a user wants to use the new animation API, they must give up the
		// old API.
		$
				.widget(
						"ui.tabs",
						$.ui.tabs,
						{
							options : {
								fx : null
							// e.g. { height: "toggle", opacity: "toggle",
							// duration: 200 }
							},
							...
							_toggle : function(event, eventData) {
								var that = this, toShow = eventData.newPanel, toHide = eventData.oldPanel, fx = this
										._getFx();
...
(function($) {
	...
	$.widget("ui.tooltip", {
		...
		_disable : function() {
			...
			this.element.find(this.options.items).andSelf().each(
					function() {
						var element = $(this);
						if (element.is("[title]")) {
							element.data("ui-tooltip-title",
									element.attr("title")).attr("title", "");
						}
					});
		},
		...
		open : function(event) {
			...
			// If the tooltip is open and we're tracking then reposition the
			// tooltip.
			// This makes sure that a tracking tooltip doesn't obscure a focused
			// element
...
(jQuery.effects || (function($, undefined) {
	 ...
	 * 
	 * Copyright 2012 jQuery Foundation and other contributors Released under
	...
	(function() {
		...
		function getElementStyles() {
			var style = this.ownerDocument.defaultView ? this.ownerDocument.defaultView
			...
			return this
					.queue(function() {
						...
						allAnimations = allAnimations
								.map(function() {
									var styleInfo = this, dfd = $.Deferred(), opts = jQuery
											.extend({}, o, {
												queue : false,
												complete : function() {
		...
		$.fn
				.extend({
					effect : function( /* effect, options, speed, callback */) {
						...
						oldEffectMethod = !effectMethod && backCompat
								&& $.effects[args.effect];
						...
						// TODO: remove this check in 2.0, effectMethod will
						// always be true
						if (effectMethod) {
					...
					toggle : function(speed) {
						if (standardSpeed(speed) || typeof speed === "boolean"
								|| $.isFunction(speed)) {
