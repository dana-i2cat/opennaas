 ...
 * ManagedSystemElements. This collection is related to the providing of compute
 * capabilities and MAY serve as an aggregation point to associate one or more
 * of the following elements: FileSystem, OperatingSystem, Processor and Memory
...
public class ComputerSystem extends System implements Serializable {
	...
	/**
	 * This constructor creates a ComputerSystemBeanImpl Class which implements
	 * the ComputerSystemBean Interface, and encapsulates the CIM class
	 * ComputerSystem in a Java Bean. The CIM class ComputerSystem is described
	 * as follows:
	 * 
	 * A class derived from System that is a special collection of
	 ...
	 * CIM. They provide the scope for numerous components. Having unique System
	 * keys is required. The NameFormat property identifies how the
	 * ComputerSystem Name is generated. The NameFormat ValueMap qualifier
	 * defines the various mechanisms for assigning the name. Note that another
	 * name can be assigned and used for the ComputerSystem that better suit a
	 ...
	 * dedicated, if any, and what functionality is provided. For example, one
	 * could specify that the System is dedicated to "Print" (value=11) or acts
	 * as a "Hub" (value=8). Also, one could indicate that this is a general
	 * purpose system by indicating 'Not Dedicated' (value=0) but that it also
	 * hosts 'Print' (value=11) or mobile phone 'Mobile User Device' (value=17)
	 * services. A clarification is needed with respect to the value 17
	 * ("Mobile User Device"). An example of a dedicated user device is a mobile
	 * phone or a barcode scanner in a store that communicates via radio
	 * frequency. These systems are quite limited in functionality and
	 * programmability, and are not considered 'general purpose' computing
	 * platforms. Alternately, an example of a mobile system that is 'general
	 * purpose' (i.e., is NOT dedicated) is a hand-held computer. Although
	 * limited in its programmability, new software can be downloaded and its
	 * functionality expanded by the user. A value of "Management" indicates
	 * this instance is dedicated to hosting system management software. A value
	 * of "Management Controller" indicates this instance represents specialized
	 * hardware dedicated to systems management (i.e., a Baseboard Management
	 * Controller (BMC) or service processor). The management scope of a
	 * "Management Controller" is typically a single managed system in which it
	 * is contained. A value of "Chassis Manager" indicates this instance
	 * represents a system dedicated to management of a blade chassis and its
	 * contained devices. This value would be used to represent a Shelf
	 * Controller. A "Chassis Manager" is an aggregation point for management
	 * and may rely on subordinate management controllers for the management of
	 * constituent parts. A value of "Host-based RAID Controller" indicates this
	 * instance represents a RAID storage controller contained within a host
	 * computer. A value of "Storage Device Enclosure" indicates this instance
	 * represents an enclosure that contains storage devices. A
	 * "Virtual Tape Library" is the emulation of a tape library by a Virtual
	 * Library System. A "Virtual Library System" uses disk storage to emulate
	 * tape libraries.A "FC Switch" indicates this instance is dedicated to
	 * switching layer 2 fibre channel frames. An "Ethernet Switch" indicates
	 * this instance is dedicated to switching layer 2 ethernet frames.
	 * 
	 ...
	 * 
	 * A string describing how or why the system is dedicated when the Dedicated
	 * array includes the value 2, "Other".
	 * 
	 ...
	 * reset is not allowed. In addition to Enabled and Disabled, other Values
	 * for the property are also defined - "Not Implemented" (5), "Other" (1)
	 * and "Unknown" (2).
	 * 
	...
	/**
	 * This method sets the ComputerSystem.resetCapability property value. This
	 * property is described as follows:
	 * 
	 * If enabled (value = 4), the ComputerSystem can be reset via hardware
	 * (e.g. the power and reset buttons). If disabled (value = 3), hardware
	...
	/**
	 * This method sets the ComputerSystem.powerManagementCapabilities property
	 * value. This property is described as follows:
	 * 
	 * An enumerated array describing the power management capabilities of the
	 * ComputerSystem. The use of this property has been deprecated. Instead,
