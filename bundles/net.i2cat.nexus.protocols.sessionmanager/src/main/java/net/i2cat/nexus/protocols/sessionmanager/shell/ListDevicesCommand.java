 ...
 * 
 */
@Command(scope = "nexus", name = "listDevices", description = "List the device ids registere do the protocol manager")
public class ListDevicesCommand extends OsgiCommandSupport {

	@Override
	protected Object doExecute() throws Exception {
		log.debug("Executing list devices shell command");

		try {
			IProtocolManager manager = getProtocolManager();
			List<String> deviceIds = manager.getAllDeviceIds();
			
			System.out.println("The following devices have registered to the protocol manager:");
