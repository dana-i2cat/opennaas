 ...
 *
 */
public class NetconfStreamReader {
	
	/** Logger */
    static private Logger logger = LoggerFactory.getLogger(NetconfStreamReader.class);
     ...
     */
    public NetconfStreamReader(InputStream stream, OutputStream oStream, List<String> prompts, 
     ...
     * @return
     */
    public NetconfResponseMessage getResponse(String request) throws IOException{
    	try {
			NetconfResponseMessage response = null;
				...
				if ((c > VALIDSTART && c < VALIDSTOP) || c == LF || c == CR) {
					...
					for (int i = 0; i < prompts.size(); i++) {
						if (currentResponse.indexOf(prompts.get(i))!= -1){
							...
							//process message
							response = NetconfResponseMessage.parse(currentResponse, stringsToRemove, errorMessagePatterns);
							buffer = new StringBuilder();
							//quit the loops
							return response;
						}
