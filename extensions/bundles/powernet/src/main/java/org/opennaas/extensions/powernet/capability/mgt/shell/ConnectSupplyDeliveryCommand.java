...
public class ConnectSupplyDeliveryCommand extends GenericKarafCommand {
	...
	@Argument(index = 2, name = "deliveryId", description = "The id of the delivery receiving energy.", required = true, multiValued = false)
	...
	protected Object doExecute() throws Exception {
		...
		try {
			...
			if (!disconnect){
				capab.connectSupplyDelivery(supplyId, deliveryId);
			} else {
				capab.disconnectSupplyDelivery(supplyId, deliveryId);
			}
		} catch (Exception e) {
