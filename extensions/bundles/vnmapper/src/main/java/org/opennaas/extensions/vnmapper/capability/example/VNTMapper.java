...
public class VNTMapper
{
	...
	public ArrayList vNodesCandidatePNodesSorting(ArrayList<VNTNodeMappingCell> VNTNodeMappingArray)
	{
		...
		for (int i = 1; i < (int) VNTNodeMappingArray.size(); i++) {
			j = 0;
			while (((VNTNodeMappingArray.get(Integer.valueOf(res.get(j).toString())).possibleRealNodes.size()) <= (VNTNodeMappingArray.get(i).possibleRealNodes
					.size())) && (j < i))
			{
				j++;
			}
			...
			if (res == 1)
			{
				...
				for (int i = 0; i < (int) VNTLinkMappingArray.size(); i++) {
					for (int j = 0; j < (int) VNTLinkMappingArray.get(i).size(); j++)
					{
						...
						{
							if (i < j) {
								linkCosts = linkCosts + v.connections.get(i).get(j).capacity * VNTLinkMappingArray.get(i).get(j).resultPath.links
										.size();

								for (int u = 0; u < VNTLinkMappingArray.get(i).get(j).resultPath.links.size(); u++)
								{
									int temp = Integer.valueOf(mappingResult.links
											.get(VNTLinkMappingArray.get(i).get(j).resultPath.links.get(u).node1Id)
											.get(VNTLinkMappingArray.get(i).get(j).resultPath.links.get(u).node2Id).toString());
									temp += v.connections.get(i).get(j).capacity;
									mappingResult.links.get(VNTLinkMappingArray.get(i).get(j).resultPath.links.get(u).node1Id).remove(
											VNTLinkMappingArray.get(i).get(j).resultPath.links.get(u).node2Id);
									mappingResult.links.get(VNTLinkMappingArray.get(i).get(j).resultPath.links.get(u).node1Id).add(
											VNTLinkMappingArray.get(i).get(j).resultPath.links.get(u).node2Id, temp);
								}
								// /
							} else
							{
								linkCosts = linkCosts + v.connections.get(j).get(i).capacity * VNTLinkMappingArray.get(j).get(i).resultPath.links
										.size();
								// /
								for (int u = 0; u < VNTLinkMappingArray.get(i).get(j).resultPath.links.size(); u++)
								{
			...
			ArrayList<ArrayList<VNTLinkMappingCell>> VNTLinkMappingArray, ArrayList sortedVNodesSet, IntSet mappedVNTNodes)
	{
		...
		if (currentVNodeId != -1) {
			...
			if (Global.PNodeChoice == 1) // / cost reduction
				VNTNodeMappingArray.get(currentVNodeId).possibleRealNodes = sortRealNode1(v, net, currentVNodeId,
						VNTNodeMappingArray.get(currentVNodeId).possibleRealNodes, selectedRealNodes);
			if (Global.PNodeChoice == 2) // / load balancing
				VNTNodeMappingArray.get(currentVNodeId).possibleRealNodes = sortRealNode2(v, net, currentVNodeId,
						VNTNodeMappingArray.get(currentVNodeId).possibleRealNodes, selectedRealNodes);
				...
				if (!selectedRealNodes.contains(currentRealNode))
				{
					...
					for (int k = 0; (k < (int) VNTNodeMappingArray.size() && (linksMappingRes == 1)); k++)
					{
						if (VNTNodeMappingArray.get(k).chosenRealNode != -1)
						{
							...
							if ((v.connections.get(VNTNodeMappingArray.get(k).vid).get(currentVNodeId).id != -1) || (v.connections
									.get(currentVNodeId).get(VNTNodeMappingArray.get(k).vid).id != -1))
							// / now try to map the virtual link
							{
								...
								resultedPath = mapVirtualLink(v, net, VNTNodeMappingArray.get(k).vid, currentVNodeId,
										VNTNodeMappingArray.get(k).chosenRealNode, currentRealNode);
					...
					if (linksMappingRes == 1) // /mapping of virtual links is done successfully
					{
						...
						for (int u = 0; u < v.vnodeNum; u++)
						{
							...
							} else if ((u > currentVNodeId) && (v.connections.get(currentVNodeId).get(u).id != -1))
							{
								VNTNodeMappingArray.get(u).connectionsNum++;
							}
						}
	...
	ArrayList sortRealNode1(VNTRequest v, InPNetwork net, int currentVNodeId, ArrayList possibleRealNodes, IntSet selectedRealNodes)
	{
		...
		while (it.hasNext())
		{
			...
			{
				if ((realNode < Integer.valueOf(possibleRealNodes.get(i).toString())) && (net.connections.get(realNode).get(
				...
				if ((realNode > Integer.valueOf(possibleRealNodes.get(i).toString())) && (net.connections.get(
	...
	ArrayList sortRealNode2(VNTRequest v, InPNetwork net, int currentVNodeId, ArrayList possibleRealNodes, IntSet selectedRealNodes)
	{
		...
		for (int i = 0; i < possibleRealNodes.size(); i++)
		{
			...
			capacityDifference.add(Integer.valueOf(possibleRealNodes.get(i).toString()),
					net.nodes.get(Integer.valueOf(possibleRealNodes.get(i).toString())).capacity - requiredCapacity);
	...
	public ArrayList matchVirtualNode(VNTRequest v, int vid, InPNetwork net)
	{
		...
		for (int i = 0; i < (int) net.nodes.size(); i++)
		{
			...
			if (((v.vnodes.get(vid).pnodeID != "") && (v.vnodes.get(vid).pnodeID.equals(net.nodes.get(i).pnodeID))) || (v.vnodes.get(vid).pnodeID
					.equals("-")))
			{
