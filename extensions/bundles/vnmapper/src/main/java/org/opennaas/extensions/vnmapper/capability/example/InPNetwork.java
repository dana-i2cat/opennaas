		...
		while ((stop != 1) && (t < max))
		{
			...
			if (passedNodes.get(currentPassedNodeIndex).linkNum < maxLinkNum)
			{
				...
				for (int i = 0; i < (int) adjacentNodes.size(); i++)
				{
					...
					{
						if (passedNodes.get(currentPassedNodeIndex).nodeId < n) {
							n1 = passedNodes.get(currentPassedNodeIndex).nodeId;
							n2 = n;
						}

						else {
							n2 = passedNodes.get(currentPassedNodeIndex).nodeId;
							n1 = n;
						}

						if ((this.connections.get(n1).get(n2).capacity >= requiredCapacity) && (this.connections.get(n1).get(n2).delay + passedNodes
								.get(currentPassedNodeIndex).delay <= requiredDelay))
						{
			...
			for (int i = 0; i < (int) adjacentNodes.size(); i++)
			{
				...
				if ((cells.get(curNode).linkNum + 1 < maxLinkNum) || (n == dst))
					if ((this.connections.get(n1).get(n2).capacity >= requiredCapacity) && (this.connections.get(n1).get(n2).delay + cells
							.get(curNode).delay <= requiredDelay))
					{
