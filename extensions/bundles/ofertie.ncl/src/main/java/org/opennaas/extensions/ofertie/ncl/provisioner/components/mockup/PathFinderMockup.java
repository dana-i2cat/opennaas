...
public class PathFinderMockup implements IPathFinder {
	...
	private static Map<String, Route> initRoutes() {
		...
		route0Path.add(links.get("h1-0->s3-2"));
		route0Path.add(xconnects.get("s3-2->s3-3"));
		route0Path.add(links.get("s3-3->s4-2"));
		route0Path.add(xconnects.get("s4-2->s4-3"));
		route0Path.add(links.get("s4-3->h2-0"));
		...
		route1Path.add(links.get("h1-1->s5-2"));
		route1Path.add(xconnects.get("s5-2->s5-3"));
		route1Path.add(links.get("s5-3->h2-1"));
		...
		route2Path.add(links.get("h1-2->s6-2"));
		route2Path.add(xconnects.get("s6-2->s6-3"));
		route2Path.add(links.get("s6-3->s7-2"));
		route2Path.add(xconnects.get("s7-2->s7-3"));
		route2Path.add(links.get("s7-3->s8-2"));
		route2Path.add(xconnects.get("s8-2->s8-3"));
		route2Path.add(links.get("s8-3->h2-2"));
		...
		route0cPath.add(links.get("h2-0->s4-3"));
		route0cPath.add(xconnects.get("s4-3->s4-2"));
		route0cPath.add(links.get("s4-2->s3-3"));
		route0cPath.add(xconnects.get("s3-3->s3-2"));
		route0cPath.add(links.get("s3-2->h1-0"));
		...
		route1cPath.add(links.get("h2-1->s5-3"));
		route1cPath.add(xconnects.get("s5-3->s5-2"));
		route1cPath.add(links.get("s5-2->h1-1"));
		...
		route2cPath.add(links.get("h2-2->s8-3"));
		route2cPath.add(xconnects.get("s8-3->s8-2"));
		route2cPath.add(links.get("s8-2->s7-3"));
		route2cPath.add(xconnects.get("s7-3->s7-2"));
		route2cPath.add(links.get("s7-2->s6-3"));
		route2cPath.add(xconnects.get("s6-3->s6-2"));
		route2cPath.add(links.get("s6-2->h1-2"));
	...
	private static Map<String, NetworkConnection> generateTopologyLinks(Map<String, Port> ports) {
		...
		link.setDestination(ports.get("s8-3"));
		link.setId(link.getSource().getId() + "->"+link.getDestination().getId());
		links.put(link.getId(), link);
		
		link = new NetworkConnection();
		link.setSource(ports.get("s8-2"));
		link.setDestination(ports.get("s7-3"));
		link.setId(link.getSource().getId() + "->"+link.getDestination().getId());
		links.put(link.getId(), link);
		
		link = new NetworkConnection();
		link.setSource(ports.get("s6-3"));
		...
		link.setDestination(ports.get("s6-2"));
		link.setId(link.getSource().getId() + "->"+link.getDestination().getId());
		links.put(link.getId(), link);
		
		//shortest route
		link = new NetworkConnection();
		link.setSource(ports.get("s5-3"));
		...
		link.setDestination(ports.get("s5-2"));
		link.setId(link.getSource().getId() + "->"+link.getDestination().getId());
		links.put(link.getId(), link);
		
		//medium route
		link = new NetworkConnection();
		link.setSource(ports.get("s4-3"));
		link.setDestination(ports.get("h2-0"));
		link.setId(link.getSource().getId() + "->"+link.getDestination().getId());
		links.put(link.getId(), link);
		
		link = new NetworkConnection();
		link.setSource(ports.get("s3-3"));
		link.setDestination(ports.get("s4-2"));
		link.setId(link.getSource().getId() + "->"+link.getDestination().getId());
		links.put(link.getId(), link);
		
		link = new NetworkConnection();
		link.setSource(ports.get("h1-0"));
		link.setDestination(ports.get("s3-2"));
		link.setId(link.getSource().getId() + "->"+link.getDestination().getId());
		links.put(link.getId(), link);
