...
public class GetMemoryUsageAction extends FloodlightAction {
	...
	Log	log	= LogFactory.getLog(GetMemoryUsageAction.class);

	@Override
	public ActionResponse execute(IProtocolSessionManager protocolSessionManager) throws ActionException {

		log.info("Executing GetMemoryUsageAction action.");
		...
		try {
			...
			MemoryUsage memoryUsage = client.getMemoryUsage();

			// create action response with an object response
			ActionResponse response = new ActionResponse();
			response.setStatus(ActionResponse.STATUS.OK);
			response.setResult(memoryUsage);

			log.debug("GetMemoryUsageAction successfully executed. Floodlight response is " + memoryUsage);

			return response;

		} catch (Exception e) {
			log.error("Error geting Memory Usage from Floodlight Controller.", e);
