...
public class GetNeighboursActionTest {
	...
	public void parseResponseTest() throws ActionException, SecurityException, NoSuchFieldException, IllegalArgumentException, IllegalAccessException {
		...
		Assert.assertEquals("Interface " + IFACE_1_NAME + " should be connected to device " + REMOTE_DEVICE_1_ID, REMOTE_DEVICE_1_ID, neighbours
				.getDevicePortMap().get(IFACE_1_NAME));
		Assert.assertEquals("Interface " + IFACE_2_NAME + " should be connected to device " + REMOTE_DEVICE_2_ID, REMOTE_DEVICE_2_ID, neighbours
				.getDevicePortMap().get(IFACE_2_NAME));
	}
