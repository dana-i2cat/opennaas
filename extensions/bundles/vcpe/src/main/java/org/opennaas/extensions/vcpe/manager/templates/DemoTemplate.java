...
public class DemoTemplate implements ITemplate {

	private static final String	TEMPLATE	= "/templates/demotemplate.properties";
	 ...
	 * 
	 */
	public DemoTemplate() throws VCPENetworkManagerException {
		try {
	...
	private List<VCPENetworkElement> generateLogicalElements(VCPENetworkModel initialModel) {
		// ----------------------------- VCPE-router1 -----------------------------
		Router vcpe1gui = (Router) VCPENetworkModelHelper
				.getElementByNameInTemplate(initialModel, VCPETemplate.VCPE1_ROUTER);

		Router vcpe1 = new Router();
		vcpe1.setName(vcpe1gui.getName());
		vcpe1.setNameInTemplate(vcpe1gui.getNameInTemplate());
		...
		List<Interface> vcpe1ifaces = new ArrayList<Interface>(3);
		vcpe1ifaces.add(inter1);
		vcpe1ifaces.add(down1);
		vcpe1ifaces.add(up1);
		vcpe1.setInterfaces(vcpe1ifaces);
		...
		// ----------------------------- VCPE-router2 -----------------------------
		Router vcpe2gui = (Router) VCPENetworkModelHelper.getElementByNameInTemplate(initialModel, VCPETemplate.VCPE2_ROUTER);

		Router vcpe2 = new Router();
		vcpe2.setName(vcpe2gui.getName());
		vcpe2.setNameInTemplate(vcpe2gui.getNameInTemplate());
		...
		List<Interface> vcpe2ifaces = new ArrayList<Interface>(2);
		vcpe2ifaces.add(inter2);
		vcpe2ifaces.add(up2);
		vcpe2.setInterfaces(vcpe2ifaces);
		...
		// Link linkDown1other = getLink(linkDown1otherId, VCPETemplate.DOWN1_LINK_AUTOBAHN, VCPETemplate.LINK_TYPE_AUTOBAHN, down1other,
		// client1other);
		//
		// Link linkDown2local = getLink(null, VCPETemplate.DOWN2_LINK_LOCAL, VCPETemplate.LINK_TYPE_ETH, down2, down2other);
		// Link linkDown2other = getLink(linkDown2otherId, VCPETemplate.DOWN2_LINK_AUTOBAHN, VCPETemplate.LINK_TYPE_AUTOBAHN, down2other,
		// client2other);
