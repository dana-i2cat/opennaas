...
public class AddInterfaceToLRCommand extends GenericKarafCommand {
	...
	@Argument(index = 2, name = "interfaces", description = "The names list of the interfaces to be transfered.", required = true, multiValued = true)
	private List<String>	subinterfaces;

	@Override
	protected Object doExecute() throws Exception {
		printInitCommand("Add interfaces to a child logical router");
		...
		try {
			...
			chassisCapability.addInterfacesToLogicalRouter(splitResourceName(logicalResourceId)[1], interfacesNamesList);
