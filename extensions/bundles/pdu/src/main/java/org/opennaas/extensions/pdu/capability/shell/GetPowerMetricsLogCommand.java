...
@Command(scope = "pdu", name = "getPowerMetricsLog", description = "Reads power metrics from specified port")
public class GetPowerMetricsLogCommand extends GenericKarafCommand {
	...
	@Argument(index = 2, name = "from", description = "Beginning date", required = true, multiValued = false)
	private String		from;

	@Argument(index = 3, name = "to", description = "End date", required = true, multiValued = false)
	private String		to;
	...
	protected Object doExecute() throws Exception {
		...
		printInitCommand("getPowerMetricsLog of port : " + portId + " in resource: " + resourceId);

		IResource resource = getResourceFromFriendlyName(resourceId);
		PowerMonitorLog log = ((IPDUPowerMonitoringIDsCapability) resource.getCapabilityByInterface(IPDUPowerMonitoringIDsCapability.class))
				.getPowerMetricsByTimeRange(portId, dateFormat.parse(from), dateFormat.parse(to));
