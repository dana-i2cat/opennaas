 ...
 *
 */
public class BGPPoliciesCreationJob{
	private RouterType routerSelected;
	private SimpleBGPPolicyType[] policiesToAdd;
	private EndpointReferenceType ipnetworkEPR;
	
	public BGPPoliciesCreationJob(RouterType routerSelected, EndpointReferenceType ipnetworkEPR, SimpleBGPPolicyType[] policiesToAdd) {
	...
	private QueueActionReq prepareRequest() throws Exception{
		...
		affectedRouters[0].setSimpleBgpPolicies(policiesToAdd);
		...
		parameters[0].setValue(EngineConstants.CONFIGURE_POLICIES_BGP_ACTION);
