 ...
 *
 */
public class RIPActivationJob{
	private RIPValues values;
	private RouterType[] routersSelected;
	private EndpointReferenceType ipnetworkEPR;
	
	public RIPActivationJob(RouterType[] routersSelected, EndpointReferenceType ipnetworkEPR, RIPValues values) {
		this.values = values;
	...
	private QueueActionReq prepareRequest() throws Exception{
		...
		actionList[0] = action;
		action.setActionID(IPNetworkConstants.INVOKE_CONFIGURE_RIP);
		...
		for(int x = 0; x<routersSelected.length; x++){
			...
			RIPRouterConfigurationType ripConfiguration = values.getRIPConfigurationByRouterName(routersSelected[x].getRouterName());
			affectedRouters[x].setRipConfiguration(ripConfiguration);
			...
			for(cat.i2cat.manticore.stubs.router.PhysicalInterfaceType phy : routersSelected[x].getPhysicalInterfaces()){
				...
				for(cat.i2cat.manticore.stubs.router.SubInterfaceType subInt : phy.getSubInterfaces()){
					if(values.checkRIPSubInterfaceConfiguration(routersSelected[x].getRouterName(), phy.getLocation(), subInt.getIdentifier())){
						SubInterfaceType subIntType = new SubInterfaceType();
						RIPSubInterfaceConfigurationType ripSubIntConfig = values.getRIPSubInterfaceConfiguration(routersSelected[x].getRouterName(), phy.getLocation(), subInt.getIdentifier());
						subIntType.setRIPSubInterfaceConfiguration(ripSubIntConfig);
