package cat.i2cat.manticore.test.demo.step5;
...
public class ActivateIBGPinAS2Test {
	static {
	...
	@Test
	public void createPoliciesandConfigureIBGPinAS2()
	{
		...
		for(RouterType router : routers){
			...
			}else if(router.getRouterName().compareTo("R-AS2-2")==0){
				routersSelected[1] = router;
			}else if(router.getRouterName().compareTo("R-AS2-3")==0){
		...
		for(int i=0; i<3; i++){
			...
			try {
				BGPPoliciesCreationJob job = new BGPPoliciesCreationJob(routersSelected[i], ipnetwork.getEpr(), policies);
		...
		String[] loopbackAddresses = new String[]{"192.168.201.1", "192.168.195.1", "192.168.190.1"};
		
		for(int i=0; i<3; i++){
			BGPValues values = new BGPValues();
			values.setType(EngineConstants.INTERNAL_BGP_TYPE);
			values.setRouterASNumber("2");
			values.setLocalAddress(loopbackAddresses[i]);
			values.setRouterID(loopbackAddresses[i]);
			int neighbor1 = 1;
			int neighbor2 = 2;
			if(i==1){
				neighbor1 = 0;
			...
			values.setNeighbors(new String[]{loopbackAddresses[neighbor1], loopbackAddresses[neighbor2]});
			
			try {
				BGPActivationJob job = new BGPActivationJob(routersSelected[i], ipnetwork.getEpr(), values);
				IPNetworkActionType[] queue = job.run();
				assertThat(queue.length, equalTo(i+1));
			} catch (Exception e) {
				e.printStackTrace();
