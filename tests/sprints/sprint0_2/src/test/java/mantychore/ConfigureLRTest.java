...
public class ConfigureLRTest extends AbstractIntegrationTest {
	...
	public void ConfigureInterfaceInterfaceTest () throws Exception {
		...
		//When you create a logical router, do you wanto to have created a resource which represents this resource!!! I think this idea is not correct
		//You should have a different command o extra flag to create this resource (in the resource:create??). Also, you have to specify its capabilities
		...
		testLogicalRouterConfigureCheckInterface(commandprocessor,"fe-0/1/3","1","192.168.13.2","255.255.255.0",logicalRouterName,(ComputerSystem)logicalResource.getModel(),isMock);
		
		/* test for ethernet interfaces */
		testLogicalRouterConfigureCheckInterface(commandprocessor,"lt-0/1/2","12","192.168.12.2","255.255.255.0",logicalRouterName,(ComputerSystem)logicalResource.getModel(),isMock);
	
		/* test for ethernet interfaces */
		testLogicalRouterConfigureCheckInterface(commandprocessor,"lo0.1","1","192.168.1.3","255.255.255.0",logicalRouterName,(ComputerSystem)logicalResource.getModel(),isMock);
		...
		List<String> response3 = KarafCommandHelper.executeCommand("chassis:addInterface " + resourceFriendlyID+ " " + logicalRouterName + " " + interfId4,
				commandprocessor);
		log.info(response3.get(0));
		
		testLogicalRouterConfigureCheckInterface(commandprocessor,"fe-0/1/3","4",null,null,logicalRouterName,(ComputerSystem)logicalResource.getModel(),isMock);		
		...
		// chassis:addInterface R1 L1 fe-0/0/1.1
		// check interface is included in the L1
		List<String> response4 = KarafCommandHelper.executeCommand("chassis:addInterface " + resourceFriendlyID+ " " + logicalRouterName + " " + interfId4,
				commandprocessor);
		...
		List<String> response5 = KarafCommandHelper.executeCommand("chassis:removeInterface " + resourceFriendlyID+ " " + logicalRouterName + " " + interfId4,
				commandprocessor);
	...
	public void testLogicalRouterConfigureCheckInterface(CommandProcessor commandprocessor,String inter, String port, String Ip, String mask,String resourceFriendlyID, ComputerSystem model, boolean isMock) throws Exception {
		// ipv4:setIP fe-0/0/1.1 192.168.1.2 255.255.255.0
		List<String> response = KarafCommandHelper.executeCommand("ipv4:setIP " + resourceFriendlyID + " "+inter+"."+port+" "+Ip+" "+mask,
				commandprocessor);
		//
		...
		if (!isMock) Assert.assertTrue(CheckHelper.checkInterface(inter,port,Ip,mask,model));
