...
public class DisableOSPFInInterfaceActionTest {
	Log											log	= LogFactory.getLog(DisableOSPFInInterfaceActionTest.class);
	private static DisableOSPFInInterfaceAction	action;
	...
	public static void init() {
		...
		action = new DisableOSPFInInterfaceAction();
	...
	public void actionIDTest() {
		...
		Assert.assertEquals("Wrong ActionID", ActionConstants.OSPF_DISABLE_INTERFACE, action.getActionID());
	}
	...
	public void executeActionTest() throws IOException {
		...
		try {
			...
			Assert.assertTrue(response.getActionID().equals("disableOSPFInInterface"));
